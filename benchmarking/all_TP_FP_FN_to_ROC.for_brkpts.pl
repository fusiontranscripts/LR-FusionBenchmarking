...
sub parse_file {
    ...
    while(my $row = $delim_parser->get_row()) {
        ...
        my $fusion = $row->{fusion} || $row->{truth_fusion_name};

        my $breakpoint = $row->{truth_brkpts} || $row->{pred_brkpts} or die "Error, need truth_brkpts || red_brkpts  column value";
        
        my $fusion_token = join("::", $progname, $sample_name, $fusion, $breakpoint);
