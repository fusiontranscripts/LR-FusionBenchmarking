...
my $truth_set = "$FindBin::Bin/SeraCare_fusion_targets.tsv";
...
sub score_and_plot {
    ...
    foreach my $sample_type (keys %$sample_to_fusion_preds_href) {
        ...
        if (! -e $sample_checkpoint) {
            &examine_sample($sample_type, $truth_set_href->{"SeraCareFusion"}, $sample_to_fusion_preds_href->{$sample_type}, $analysis_settings_href, $column_headers_aref);
